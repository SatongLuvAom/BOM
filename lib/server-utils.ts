...
// Format: TPL-YYMMDD-0001
export async function generateTemplateId(): Promise<string> {
  const supabase = await createClient()
  const now = new Date()
  const yy = String(now.getFullYear()).slice(2)
  const mm = String(now.getMonth() + 1).padStart(2, '0')
  const dd = String(now.getDate()).padStart(2, '0')
  const prefix = `TPL-${yy}${mm}${dd}-`
  ...
  let seq = 1
  if (data && data.length > 0) {
    seq = parseInt(data[0].template_id.split('-').pop() ?? '0', 10) + 1
  }
  return `${prefix}${String(seq).padStart(4, '0')}`
}
...
export async function generateCustomerId(): Promise<string> {
  ...
  const { data } = await supabase
    .from('customer')
    .select('customer_id')
    .like('customer_id', 'CUS-%')
    .order('customer_id', { ascending: false })
    .limit(1)

  let seq = 1
  if (data && data.length > 0) {
    seq = parseInt(data[0].customer_id.replace('CUS-', ''), 10) + 1
  }
  return `CUS-${String(seq).padStart(4, '0')}`
}

// ── BOQ Project ID Generator ──────────────────────────────────
// Format: BOQ-YYMMDD-0001
export async function generateBoqId(): Promise<string> {
  const supabase = await createClient()
  const now = new Date()
  const yy = String(now.getFullYear()).slice(2)
  const mm = String(now.getMonth() + 1).padStart(2, '0')
  const dd = String(now.getDate()).padStart(2, '0')
  const prefix = `BOQ-${yy}${mm}${dd}-`
  ...
  let seq = 1
  if (data && data.length > 0) {
    const lastSeq = parseInt(data[0].project_id.split('-').pop() ?? '0', 10)
    seq = lastSeq + 1
  }
