...
function receiptError(error: unknown) {
  ...
  if (isReceiptSchemaMissing(error)) {
    return databaseError('Could not auto-match receipt item materials. Run sql/phase2b_receipt_import_v1.sql in Supabase first.', error)
  }

  return databaseError('Receipt material auto-match failed', {
...
export async function POST(_req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const data = await autoMatchReceiptItemMaterials(supabase, id, owner.id)
